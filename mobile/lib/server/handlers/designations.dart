...
Router designationsRouter(AppDatabase db) {
  ...
  // GET /designations — list all (excludes sentinel id=0, summary only)
  router.get('/', (Request req) async {
    final list = await db.listDesignations();
    final filtered = list.where((d) => d.id != 0).toList();
    return _json(200, filtered.map((d) => d.toSummaryJson()).toList());
  });

  // GET /designations/<id> — full detail (includes description + picture)
